...
async def query(request: QueryRequest, current_user: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if request.stream:
        return _stream_query(prompt, embeddings, results, is_hit, system_prompt, model, temperature)
...
def _stream_query(prompt, embeddings, results, is_hit, system_prompt, model, temperature):
    async def generator():
        active_requests.inc()
        try:
            if is_hit:
                cache_hits.labels(method="POST", endpoint="/query/").inc()
                yield f"data: {json.dumps({'token': results[0]['response'], 'cache_hit': True})}\n\n"
                yield "data: [DONE]\n\n"
            else:
                cache_misses.labels(method="POST", endpoint="/query/").inc()
                llm_requests.labels(method="POST", endpoint="/query/").inc()
